...
// providerHeaderConfig defines how to authenticate with a model-list endpoint.
type providerHeaderConfig struct {
	defaultBaseURL string
	// header is a list of (key, valueTemplate) pairs where valueTemplate
	// receives the apiKey if non-empty, otherwise falls back to the
	// env-resolved key for the given core.KnownProvider.
	header []struct{ key, valueTemplate string }
}

// providerModelListConfigs maps provider string to its header config.
...
func (a *App) GetModels(params map[string]interface{}) ([]ModelInfo, error) {
	...
	return fetchModelList(providerStr, baseURL, apiKey)
}

// fetchModelList calls the /models endpoint of the given provider.
func fetchModelList(provider, baseURL, apiKey string) ([]ModelInfo, error) {
	...
	// If apiKey was empty and the header template used it,
	// the Authorization header was set to "Bearer ". Try env fallback.
	if apiKey == "" {
...
// getCachedModels returns built-in models for a provider without making
// an HTTP request.
