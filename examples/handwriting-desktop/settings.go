...
func NewApp() *App {
	...
	if err := os.MkdirAll(fontDir, 0755); err != nil {
		LogError("[app] failed to create font dir %s: %v", fontDir, err)
	}
	return &App{
		dataDir:       dataDir,
		fontAssetsDir: fontDir,
...
// startup 在 Wails 启动时调用：初始化日志、注册 provider、加载设置、释放内置字体。
func (a *App) startup(ctx context.Context) {
	...
	LogInfo("=== Handwriting Desktop starting, data dir: %s ===", a.dataDir)

	providers.RegisterBuiltInProviders()
	a.loadSettings()
	a.releaseBuiltinFonts()
}

// getDataDir 返回应用数据目录 (~/.handwriting-desktop)。
func getDataDir() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ".handwriting-desktop"
	}
	return filepath.Join(home, ".handwriting-desktop")
}
