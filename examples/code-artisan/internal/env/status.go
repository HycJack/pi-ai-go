...
func EvaluateStatus(requirements []Requirement) (Status, error) {
	...
	case !archiveExists && len(missing) > 0:
		...
		status.Summary = "缺少 resources/runtime/runtime.zip，无法自动解压 Python 运行环境"
		status.RecommendedAction = "请将 WinPython 打包为 resources/runtime/runtime.zip"
	case !pythonExists:
		status.Code = "python_executable_missing"
		status.Severity = "error"
		status.Summary = "Python 解释器缺失"
		status.RecommendedAction = "可以执行重建 Runtime 解压"
	case len(missing) > 0:
		status.Code = "runtime_incomplete"
		status.Severity = "error"
		status.Summary = "Python 环境不完整"
	...
	default:
		...
		case importErr != nil:
			...
			status.Severity = "error"
			status.Summary = "Python 可执行但导入自检失败"
			status.RecommendedAction = "建议重建 Runtime"
		case len(importMissing) > 0:
			status.Ready = false
			status.Code = "python_package_unhealthy"
			status.Severity = "error"
			status.Summary = "核心包导入失败"
			status.RecommendedAction = "建议重建 Runtime 以修复包依赖"
		default:
			status.Code = "ready"
			status.Severity = "info"
			status.Summary = "WinPython 运行环境就绪"
